...
function isPremiumRoute(route) {
  if (!route) return false
  return route.includes('CN2') || route.includes('9929') || route.includes('CMIN2')
}
            ...
            <template #default="{ row }">
              <template v-if="speedtests[row.id]">
                ...
                <div v-if="hasRecognizedRoutes(speedtests[row.id])" class="route-tags">
                  <span v-if="isValidRoute(speedtests[row.id].telecom_route)" :class="['route-tag', isPremiumRoute(speedtests[row.id].telecom_route) ? 'route-premium' : 'route-normal']">
                    {{ speedtests[row.id].telecom_route }}
                  </span>
                  <span v-if="isValidRoute(speedtests[row.id].unicom_route)" :class="['route-tag', isPremiumRoute(speedtests[row.id].unicom_route) ? 'route-premium' : 'route-normal']">
                    {{ speedtests[row.id].unicom_route }}
                  </span>
                  <span v-if="isValidRoute(speedtests[row.id].mobile_route)" :class="['route-tag', isPremiumRoute(speedtests[row.id].mobile_route) ? 'route-premium' : 'route-normal']">
