...
const filteredFirewallRules = computed(() => firewallRules.value.filter((row) => {
  ...
  return includesText([row.node_name, row.action, row.protocol, row.cidr, row.location, row.port], keyword.value)
}))
const filteredJails = computed(() => jails.value.filter((row) => {
  if (selectedNode.value && row.node_id !== selectedNode.value) return false
  if (selectedStatus.value && String(row.enabled) !== selectedStatus.value) return false
  return includesText([row.node_name, row.name, row.log_path, row.filter_name], keyword.value)
}))
...
<template>
  <div class="page-shell">
    <PageHeader title="网络防护"><el-button :icon="Refresh" @click="load">刷新</el-button></PageHeader>
