...
访问规则只负责分流，不再提供任何预定义规则。规则可以使用“表格配置”或“高级纯文本”模式，按 Mihomo 官方规则从上到下匹配；必须由操作者手动配置最后一条 `MATCH`，系统不会补写默认动作或终结规则。创建客户端配置只保存配置，不自动下载；保存后仍可以在列表中手动下载 YAML、复制更新地址或轮换更新地址。

生成的 DNS 仅使用 DoH：`https://223.5.5.5/dns-query`、`https://dns.alidns.com/dns-query`、`https://doh.pub/dns-query`，以及用于代理节点域名解析的 `proxy-server-nameserver`。不会写入明文 UDP/TCP DNS 服务器。
